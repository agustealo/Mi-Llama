...
async function activateCurrentManuscript() {
  ...
  if (context.editor.getDocumentState()?.schema === 'tiptap_v1') {
    ensureFormattingToolbar(context.editor, context.editor.sourceElement()?.readOnly)
    return
  }
  ...
  try {
    ...
    const readOnly = Boolean(source?.readOnly || !draft.version)
    if (draft.editor_state?.schema === 'tiptap_v1' || readOnly) {
